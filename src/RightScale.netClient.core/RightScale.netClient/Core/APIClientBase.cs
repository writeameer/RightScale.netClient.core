...
namespace RightScale.netClient.Core
{
    ...
    public abstract class APIClientBase : IDisposable
    {
        ...
        /// <summary>
        /// System.Net.Http.HttpClient object used for executing all HTTP calls to RightScale API
        /// </summary>
        ...
        public void InitWebClient()
        {
            ...
            if (string.IsNullOrWhiteSpace(apiBaseAddress) && ConfigurationManager.AppSettings["RightScaleAPI_BaseAddress"] != null)
            {
            ...
            if (!webClient.DefaultRequestHeaders.Any( x => x.Key == "X_API_Version"))
               webClient.DefaultRequestHeaders.Add("X_API_Version", apiVersion);
        }
        ...
        {
            var authTimeoutMills = authTimeoutMins * 60 * 10000;
            var callback = new TimerCallback(authTimer_Elapsed);
            Debug.WriteLineIf(outputDebug, "Authentication timer being set up to tick in " + authTimeoutMins + " minutes to force reset of authentication properties of this object later", TraceDebugCategory.RSAPIAuthenticate);
            authTimer = new Timer(callback, null, authTimeoutMills  , authTimeoutMills); // 118 mins to account for a 120 min session timeout
        }
        ...
        /// <param name="e"></param>
        void authTimer_Elapsed(object state)
        {
            authTimer.Dispose();
            ...
            Console.WriteLine("authTimer_Elapsed");
            InitWebClient();
