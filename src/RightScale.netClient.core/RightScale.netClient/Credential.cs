...
namespace RightScale.netClient
{
    ...
    /// </summary>
    public class Credential : Core.RightScaleObjectBase<Credential>
    {
        ...
        {
            string jsonString = Core.APIClient.Instance.Get(string.Format(APIHrefs.Credential, Core.API10Client.Instance.accountId));

            return Credential.deserializeList(jsonString);
        }
        ...
        {
            string jsonString = Core.API10Client.Instance.Get(string.Format(APIHrefs.CredentialByID, Core.API10Client.Instance.accountId, credentialID));
            return Credential.deserialize(jsonString);
        }
        ...
        public static string create(string name, string value, string description)
        {
            ...
            Utility.addParameter(value, "credential[value]", putParams);
            Utility.addParameter(description, "credential[description]", putParams);
            return Core.APIClient.Instance.Post(postHref, putParams, "Location").Last<string>().Split('/').Last<string>();
        }
    }
}
